module.exports = function(req, res, callback) {
	if (req.method == 'GET') {
		...
		var skip = {};
		var limit = {};
		var setreadtrue = false; //setreadtrue islemi yapilacak

		if (memberid != '') {
			match = {
				$match: {
					memberid: ObjectId(authinfo.data._id),
					memberid2: ObjectId(memberid)
				}
			};
		...
		db.messages.aggregate(aggregation).exec(function(err, docs) {
			...
			db.messages.populate(docs, {
				path: "memberid2",
				select: 'membertype employee firm mainpicture mainpictureblur showname showpicture profileenabled'
