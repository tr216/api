...
function getList(activeDb,member,req,res,callback){
    ...
    activeDb.production_break_reasons.paginate(filter,options,(err, resp)=>{
...
function getOne(activeDb,member,req,res,callback){
    activeDb.production_break_reasons.findOne({_id:req.params.param1},(err,doc)=>{
        if (!err) {
...
function post(activeDb,member,req,res,callback){
    ...
    var newdoc = new activeDb.production_break_reasons(data);
...
function put(activeDb,member,req,res,callback){
    if(req.params.param1==undefined){
        ...
        activeDb.production_break_reasons.findOne({ _id: data._id},(err,doc)=>{
            if (!err) {
                if(doc==null){
                    callback({success: false,error: {code: 'RECORD_NOT_FOUND', message: 'Kayit bulunamadi'}});
                }else{
                    var doc2 = Object.assign(doc, data);
                    var newdoc = new activeDb.production_break_reasons(doc2);
                    var err=epValidateSync(newdoc);
                    if(err) return callback({success: false, error: {code: err.name, message: err.message}});
                    newdoc.save(function(err, newdoc2) {
                        if (!err) {
...
function deleteItem(activeDb,member,req,res,callback){
    if(req.params.param1==undefined){
        ...
        var data = req.body || {};
        data._id = req.params.param1;
        activeDb.production_break_reasons.removeOne(member,{ _id: data._id},(err,doc)=>{
            if (!err) {
