class DespatchIntegration {
    ...
    run(funcName,parameters,callback){
    	this.client.send(funcName,parameters,(err,data)=>{
    		if(!err){
    			var obj=mrutil.renameInvoiceObjects(data[`${funcName}Response`][`${funcName}Result`],mrutil.renameKey)
...
class DespatchInfo {
    ...
    generateXml(){
    	...
    	var xmlDespatchAdvice=mrutil.e_despatch2xml(this.DespatchAdvice,'DespatchAdvice')
