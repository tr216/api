			...
			dbModel.invoices.findOne({ioType:0, localDocumentId:{$ne:''},localDocumentId:invoices[index].localDocumentId},(err,doc)=>{
				if(!err){
					if(doc==null){
						...
						var data1=mrutil.eDocumentSetCurrencyIDs(tempInvoice,tempInvoice.documentCurrencyCode.value)
			...
			dbModel.despatches.findOne({ioType:0, localDocumentId:{$ne:''},localDocumentId:despatches[index].localDocumentId},(err,doc)=>{
				if(!err){
					if(doc==null){
						...
						tempDespatch=mrutil.deleteObjectFields(tempDespatch,["_id","__v","createdDate","modifiedDate",'eIntegrator', "pdf", "html"])
						tempDespatch=mrutil.deleteObjectProperty(tempDespatch,'_id')
                        ...
                        //var data1=mrutil.eDocumentSetCurrencyIDs(tempDespatch,tempDespatch.documentCurrencyCode.value)
