...
exports.yeniFaturaNumarasi=function(dbModel,eIntegratorDoc,newDespatch,cb){
    ...
    dbModel.e_despatches.find({ioType:0, 'ID.value':{'$regex': eIntegratorDoc.despatchPrefix + yil + '.*','$options':'i'} }).sort({'ID.value':-1}).limit(1).exec((err,docs)=>{
        if(!err){
            var yeniNo=0;
...
exports.insertEDespatch=function(dbModel,eIntegratorDoc,connectorResult,callback){
    try{
        ...
        function kaydet(cb){
            ...
            dbModel.e_despatches.findOne({ioType:0, localDocumentId:{$ne:''},localDocumentId:despatches[index].localDocumentId},(err,doc)=>{
                if(!err){
                    if(doc==null){
                        var tempInvoice=(new dbModel.e_despatches(despatches[index])).toJSON();
                        ...
                        var newEDespatch=new dbModel.e_despatches(data1);
...
exports.findDefaultEIntegrator=function(dbModel,eIntegratorId,callback){
    ...
    dbModel.e_integrators.find(filter).populate(['invoiceXslt','invoiceXsltFiles']).exec((err,docs)=>{
        if(!err){
