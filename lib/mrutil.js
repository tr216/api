...
function einvoiceXmlHazirla_eski(obj){
    if(Array.isArray(obj)){
        var newObj=[];
        for(var i=0;i<obj.length;i++){
            newObj.push(einvoiceXmlHazirla(obj[i]));
        ...
        if(keys.indexOf('value')>-1){
            if(keys.indexOf('attr')>-1){
                newObj['@']={}
                var keysAttr=Object.keys(obj['attr']);
                keysAttr.forEach((keyAtt)=>{
                    newObj['@'][keyAtt]=obj['attr'][keyAtt]; 
                });
            }
            newObj['#']=obj['value'];
            ...
            keys.forEach((key)=>{
                if(key=='attr'){
                    newObj['@']={}
                    var keysAttr=Object.keys(obj[key]);
                    keysAttr.forEach((keyAtt)=>{
                        newObj['@'][keyAtt]=obj[key][keyAtt]; 
                    });
                }else{
                    var key2=key;
                    if((Array.isArray(obj[key]) || typeof obj==='object' )){
