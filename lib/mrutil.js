...
exports.simdi = function()
{
    var s= '[' + (new Date()).yyyymmddhhmmss() + ']';
    return s;
}
...
console.log = function( obj, ...placeholders ){
    if ( typeof obj === 'string' )
        placeholders.unshift( exports.simdi() + " " + obj );
    else
    {
        // This handles console.log( object )
        placeholders.unshift( obj );
        placeholders.unshift( exports.simdi() + " %j" );
    }
...
exports.simdiHata = function()
{
    var s= '[' + (new Date()).yyyymmddhhmmss() + '] Error:';
    return s;
}
...
console.error = function( obj, ...placeholders ){
    ...
    if ( typeof obj === 'string' )
        placeholders.unshift( (exports.simdiHata() + " " + obj).cyan );
    else
    {
        // This handles console.log( object )
        placeholders.unshift( obj );
        placeholders.unshift( exports.simdiHata().cyan + " %j" );
    }
    try{
        var callerFileName=_getCallerFile();
        
        if(callerFileName) {
            origError.call(console, callerFileName);
        }
        // if(config.error_log.active){
        //     fs.appendFileSync(config.error_log.path, errorText,'utf-8');
        // }
    }catch(tryErr){}
...
exports.errorLog = function (label,err) {
    ...
    //         fs.mkdirSync(dir);
    //     }
    //     var errorMsg='[' + (new Date).yyyymmddhhmmss() + '] ' + label;
    //     errorMsg +=JSON.stringify(err,null,4);
    //     errorMsg +='\r\n--------------------\r\n\r\n';

    //     fs.appendFileSync(path.join(dir,'error_log_' + (new Date).yyyymmdd() + '.txt'),errorMsg,'utf-8');
...
exports.log = function (label,text) {
    ...
    //         fs.mkdirSync(dir);
    //     }
    //     var msg='[' + (new Date).yyyymmddhhmmss() + '] ' + label;
    //     if(typeof text === 'string' || text instanceof String){
    //         msg += ' - ' + text;
    //     }else{
    //         msg +=JSON.stringify(text,null,4);
    //     }
    //     msg +='\r\n--------------------\r\n\r\n';

    //     fs.appendFileSync(path.join(dir,'log_' + (new Date).yyyymmdd() + '.txt'),msg,'utf-8');
...
exports.sendmail = function (mailto,subject, body,callback){
    try {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            ...
            if (error) {
                console.log('error:' + JSON.stringify(error));
                callback({success:false,error:{code:'SEND_EMAIL',message:error.code}});
            }else{
                exports.console('Mailer // Message sent: ' + info.response);
        ...
        callback({success:false,error:{code:'SEND_EMAIL',message:err}});
    }
}
...
exports.socketread=function(data){
  ...
  // return exports.decrypt(data.toString());
}
...
// global.Amount=(val)=>{
//     this.val=Number.parseFloat(val).toFixed(2);
// }
// global.Amount=function(val){
...
//     return this.val;
// }
// //Amount.prototype=Number.prototype;

// // var bb=new Amount(895.678);
// var bb=Amount(895.678);
// console.log(typeof bb);
// console.log(bb);
// process.exit(0); //qwerty
...
exports.deleteObjectProperty=function(obj,propertyName){
    ...
    if(Array.isArray(obj)){
        // console.log('typeof obj: array[] length:',obj.length);
        var newObj=[];
        for(var i=0;i<obj.length;i++){
        ...
        keys.forEach((key)=>{
            // console.log('key:',key);
            if(Array.isArray(obj[key]) || typeof obj[key]==='object'){
                // console.log('typeof obj:',(typeof obj),key);
...
exports.e_invoice2xml=function(invoiceObject,rootName='Invoice'){
    ...
    if(jsObject.invoiceLine!=undefined){
        console.log('jsObject.invoiceLine.length:',jsObject.invoiceLine.length)
            ...
            if(line.taxTotal!=undefined){
                if(line.taxTotal.taxAmount!=undefined){
                    if(line.taxTotal.taxAmount.value==0 && line.taxTotal.taxSubTotal==undefined){
                        ...
                        console.log('delete line.taxTotal;');
                    }
                }
            }
        });
...
exports.e_invoiceXslt=function(jsObject){
    // var fileName=path.join(__dirname,'../../temp','e_invoice2xml');
    // console.log(fileName);
    // var xmlString=exports.e_invoice2xml(jsObject);
    var xsltString='';
    // var html='';
...
function einvoiceRenameKeys(key){
    ...
    if(Array.isArray(obj)){
        ...
        keys.forEach((key)=>{
            if(key!='attr' && key!='value'){
                var key2=key;
                if((Array.isArray(obj[key]) || typeof obj[key]==='object' )){
                    key2='cac:' + einvoiceRenameKeys(key);
                    if(typeof obj[key]==='object'){
                        if(Object.keys(obj[key]).indexOf('value')>-1){
                            key2='cbc:' + einvoiceRenameKeys(key);
                        }
                    }
...
exports.eInvoiceSetCurrencyIDs=function(obj,currencyID){
    ...
    if(Array.isArray(obj)){
        //var newObj=[];
        for(var i=0;i<obj.length;i++){
            //newObj.push(exports.eInvoiceSetCurrencyIDs(obj[i],currencyID));
            obj[i]=exports.eInvoiceSetCurrencyIDs(obj[i],currencyID);
        }
        return obj;
    }else if (typeof obj==='object'){
        //var newObj={};
            ...
            // if(obj[key]!=null && typeof obj[key]!='function' && typeof obj[key]!='ObjectID' && key.indexOf('$')<0  && key.indexOf('_')<0){
                if(key!='attr' && key!='value'  && key!='_id'  && key!='__v' ){ //&& key.indexOf('$')<0  && key.indexOf('_')<0
                    if((Array.isArray(obj[key]) || typeof obj[key]==='object' )){
                        ...
                        obj[key]=exports.eInvoiceSetCurrencyIDs(obj[key],currencyID);
                    }
        ...
        keys.forEach((key)=>{
            if(Array.isArray(obj[key]) || typeof obj[key]=='object'){
                ...
                if(parentKeyName.toLowerCase().indexOf('amount')>-1){
                    ...
                    if(key=='value'){
                        if(!isNaN(obj[key])){
                            console.log(parentKeyName,key,obj[key]);
                            obj[key]=Number(obj[key]).toFixed(2);
                            console.log('duzeltildi:',parentKeyName,key,obj[key]);
                        }
