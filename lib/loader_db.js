...
function LoadModules(folder,callback) {
    ...
    fs.readdir(folder, function(err, files) {
        ...
        files.forEach((e)=>{
        	f = path.join(folder, e)
        	let fileName=path.basename(f)
        	let suffix='.collection.js'
        	let apiName=fileName.substring(0,fileName.length-suffix.length)
...
global.loadUserDb=(_id,userDb,userDbHost,dbName,cb)=>{
    ...
    var folder = path.join(rootDir, 'db','userdb')
    ...
    usrConn.on('connected', function () {  
        eventLog('userDb ' + dbName.brightGreen + ' connected.')

	    fs.readdir(folder,(err,repoFiles)=>{
	        if(!err){
	        	repoFiles.forEach((e)=>{
	        		f = path.join(folder, e)
	                let fileName=path.basename(f)
    ...
    dbconn.on('error',function (err) {  
        if(tryConnectionCount<5){
            setTimeout(()=>{
                ...
                console.log('tryConnectionCount:',tryConnectionCount)
                global.dbconn = mongoose.createConnection(config.mongodb.address,{ useNewUrlParser: true ,useUnifiedTopology:true, autoIndex: true  })
            },3000)
        }else{
