    ...
    usrConn.on('connected', function () {  
        ...
        fs.readdir(path,(err,repoFiles)=>{
            if(!err){
                ...
                console.log('userDb ' + dbName.brightGreen + ' loaded.');
                if(typeof services!='undefined'){
                    console.log('Yeni veri ambari servisleri baslatiliyor');
                    repoDb[_id]['runUserDbServices']=services.runUserDbServices;
                    repoDb[_id].runUserDbServices();
                }
                cb(null);
            }else{
                console.error('Mongoose user connection "' + userDbHost + userDb + '" error: ', err);
                cb(err);
            }
    ...
    db.dbdefines.find({deleted:false,passive:false},(err,docs)=>{
        if(!err){
            ...
            kontrolet((err)=>{
                ...
                // console.log('veriAmbarlari.length:',veriAmbarlari.length);
                // console.log('startFunc:',startFunc);
                // console.log('endFunc:',endFunc);
                callback(err);
            });
...
//         if(err){
//             console.log('RESONANCE ID uretilemedi  Error:' + err.name + ' - '  + err.message);
//         }else{
//             if(c<1000){

//             }
//             console.log('generate_ResonanceIDs:' + doc);
//         }
//     });
// }
