...
global.dbconn = mongoose.createConnection(config.mongodb.address,{ useNewUrlParser: true });
...
process.on('SIGINT', function() {  
  mongoose.connection.close(function () { 
    mrutil.console('Mongoose default connection disconnected through app termination'); 
    process.exit(0); 
  }); 
}); 
    ...
    fs.readdir(path, function(err, files) {
        ...
        for (var i = 0; i < l; i++) {
            ...
            if(apiName!='' && (apiName + suffix)==fileName){
                mrutil.console('collection ' + apiName.green + ' loaded.' + f);
...
global.loadUserDb=(_id,userDb,userDbHost,cb)=>{
    if(repoDb[_id]!=undefined){
        console.log('if(userDb[_id]!=undefined){');
        return cb(null);
    }
    var path = path_module.join(__dirname, '../db/userdb');
    var usrConn = mongoose.createConnection(userDbHost + userDb,{ useNewUrlParser: true });
    usrConn.on('connected', function () {  
        mrutil.console('Mongoose user connection open to ' + userDbHost + userDb);
        // usrConn.model.removeOne=(filter,cb)=>{
        //     usrConn.model.findOne(filter,(err,doc)=>{
        //         if(!err){
        //             console.log('removeOne => silinen kayit:',doc);
        ...
        //         cb(err,doc);
        //     });
        //     //this.deleteOne(filter,cb);
        // }
        var models={};
        var repoFiles=fs.readdirSync(path);
        for(var i=0;i<repoFiles.length;i++){
            ...
            var apiName=fileName.substring(0,fileName.length-suffix.length);
            if(apiName!='' && (apiName + suffix)==fileName){
                mrutil.console('userDb collection ' + apiName.green + ' loaded.');
    ...
    usrConn.on('error',function (err) {  
        mrutil.console('Mongoose user connection "' + userDbHost + userDb + '" error: ' + err);
        cb(err);
    }); 
}

function init_userdb(cb){
    db.dbdefines.find({deleted:false,passive:false},(err,docs)=>{
        if(!err){
            ...
            function baglan(cb){
                ...
                loadUserDb(docs[index]._id,docs[index].userDb,docs[index].userDbHost,(err)=>{
                    if(!err){
                        index++;
...
module.exports=(cb)=>{
    ...
    dbconn.on('connected', function () { 
        mrutil.console('Mongoose default connection open to ' + config.mongodb.address);
    ...
    dbconn.on('error',function (err) {  
      mrutil.console('Mongoose default connection error: ' + err);
      cb(err);
    }); 

    dbconn.on('disconnected', function () {  
      mrutil.console('Mongoose default connection disconnected'); 
    });
}
// function generate_ResonanceIDs(){
//     db.resonanceids.find({password:'',passive:true}).count(function(err,c){
//         if(err){
//             mrutil.console('RESONANCE ID uretilemedi  Error:' + err.name + ' - '  + err.message);
//         }else{
//             if(c<1000){

//             }
//             mrutil.console('generate_ResonanceIDs:' + doc);
//         }
//     });
// }
