				...
				if (popdocs != null) {
					...
					function duzenle(cb) {
						...
						if (item.memberid.membertype == 'e') {
							if (showname == false) {
								item.who = util.nameMask(item.memberid.employee.name + ' ' + item.memberid.employee.lastname)
							} else {
							...
							if (showname == false) {
								item.who = util.nameMask(item.memberid.firm.firmtitle)
							} else {
